...
do
  for i in {1..5}
  do
    CUDA_VISIBLE_DEVICES=$dev python3 marginal_tails.py --dim 50 --marginals "mTAF(fix)" --flow nsf --model_nr $i --num_heavy $num_heavy --df 2 --num_layers $num_layers --tail_bound $tail_bound --seed $seed --num_blocks 2 --wandb_comment $wandb_comment
    CUDA_VISIBLE_DEVICES=$dev python3 marginal_tails.py --dim 50 --marginals "mTAF(fix)" --flow nsf --model_nr $i --num_heavy $num_heavy --df 3 --num_layers $num_layers --tail_bound $tail_bound --seed $seed --num_blocks 2 --wandb_comment $wandb_comment
